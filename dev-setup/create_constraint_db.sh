#! /usr/bin/env bash
# create_constraint_db
# Copyright (C) 2021 David Rosenberg <dmr@davidrosenberg.me>
# Distributed under terms of the GPLv3 license.
#
# Usage: create_constraint_db [-vhq] [--debug] [--sqlite] <database>
#
#
# Arguments:
...
if [[ ${ARGS[--sqlite]} == true ]]; then
  if [ -a ${ARGS[<database>]} ]; then
    echo "${ARGS[<database>]} exists! Exiting"
    exit 3
  else
    python3 -c "import loader; loader.main_sqlite('${ARGS[<database>]}')"
  fi
else
